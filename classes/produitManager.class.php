...
class produitManager {
    ...
    {        
        ...
        // le produit que nous essayons de sauvegarder existe-t-il dans la  base de données ?
        if ($prod->getId()!=''){
            $query = "SELECT COUNT(*) AS nb from `produit` WHERE `idProduit`=?";
        ...
        // Si le produit que nous essayons de sauvegarder existe dans la base de données : UPDATE
        if ($nbRows > 0)
        {
            $query = "UPDATE `produit` SET `nomProduit`=?, `descProduit`=?, `prixProduit`=?, `dureeGarantie`=?, idCategorie = ? WHERE `idProduit`=?;";
            $traitement = $this->db->prepare($query);
            $param1=$prod->getLib();
            $traitement->bindparam(1,$param1);
            $param2=$prod->getDesc();
            ...
            $param4=$prod->getGarantie();
            $traitement->bindparam(4,$param4);
            $param5=$prod->getCategorie()->getId();
        ...
        else
        {
            ...
            $query = "INSERT INTO produit(`idProduit`, `nomProduit`, `descProduit`,`prixProduit`, `dureeGarantie`, idCategorie) VALUES (?,?,?,?,?,?);";
            $traitement = $this->db->prepare($query);
            $param1=$prod->getId();
            $traitement->bindparam(1,$param1);
            $param2=$prod->getLib();
            ...
            $param4=$prod->getPrix();
            $traitement->bindparam(4,$param4);
            $param5=$prod->getGarantie();
            $traitement->bindparam(5,$param5);
            $param6=$prod->getCategorie()->getId();
            $traitement->bindparam(6,$param6);
            $traitement->execute();
        }
        return $prod->getId();
    ...
    public function getList($restriction='WHERE 1')
    {
        ...
        while ($row = $result->fetch())
        {
            ...
            $categ = $managerCateg->get($row['idCategorie']);
            //appel du constructeur paramétré
            $produit = new produit($row['idProduit'],$row['nomProduit'],$row['descProduit'],$row['prixProduit'],$row['dureeGarantie'], $categ);            
