...
class clientManager {
    ...
    {        
        ...
        if ($cli->getId()!=''){
            $query = "select count(*) as nb from `client` where `idClient`=?";
        ...
        if ($nbRows > 0)
        {
            $query = "update `client` set `nomClient`=?, `prenomClient`=?, `rueClient`=?, `cpClient`=?, `villeClient`=?, `mailClient` = ?, `telClient` = ? where `idClient`=?;";
            $traitement = $this->db->prepare($query);
            $param1=$cli->getNom();
            $traitement->bindparam(1,$param1);
        ...
        {
            $query = "insert into `client` (`nomClient`, `prenomClient`,`rueClient`,`cpClient`,`villeClient`, `mailClient`, `telClient`) values (?,?,?,?,?,?,?);";
            $traitement = $this->db->prepare($query);
            $param1=$cli->getNom();
            $traitement->bindparam(1,$param1);
            $param2=$cli->getPrenom();
    ...
    public function delete(client $cli)
    {
        ...
        {            
            ...
            $query = "DELETE FROM contenir WHERE idCommande IN "
            ...
            $query = "DELETE FROM commande WHERE idClient=?;";
            $traitement = $this->db->prepare($query);
            $param1 = $cli->getId();
            $traitement->bindparam(1,$param1);
            $traitement->execute();
    ...
    public function getList($restriction='WHERE 1')
    {
        ...
        while ($row = $result->fetch())
        {
            ...
            $cli = new client($row['nomClient'],$row['prenomClient'],$row['rueClient'],$row['cpClient'],$row['villeClient'],$row['mailClient'],$row['telClient']);
            //positionnement de l'id
            $cli->setId($row['idClient']);
